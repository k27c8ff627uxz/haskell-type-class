...
private
  ...
  record Conditions {i} {F : Set i → Set i} (body : Body F) : Prop (lsuc i) where
    FunctorOfWithUnit₀ : Functor F
    FunctorOfWithUnit₀ = Body.FunctorOfWithUnit body
    unit₀ : {A : Set i} → A → F A
    unit₀ = Body.unit body
  ...
  Body-Explicit {i} {F} functor-of-with-unit₀ unit₀ = record { FunctorOfWithUnit = functor-of-with-unit₀ ; unit = \{A : Set i} → unit₀ A }
  ...
  to-Body record { FunctorOfWithUnit = FunctorOfWithUnit₀ ; unit = unit₀ } = record { FunctorOfWithUnit = FunctorOfWithUnit₀ ; unit = unit₀ }

  to-Body-Eq : {i : Level} → {F : Set i → Set i} → (body₁ body₂ : Body F) → (Body.FunctorOfWithUnit body₁ ＝ Body.FunctorOfWithUnit body₂) → ({A : Set i} → Body.unit body₁ {A} ＝ Body.unit body₂ {A}) → body₁ ＝ body₂
  to-Body-Eq
    {i} {F}
    record { FunctorOfWithUnit = functor₁ ; unit = unit₁ }
    record { FunctorOfWithUnit = functor₂ ; unit = unit₂ }
    eq-functor eq-unit
    = ＝-begin
      record { FunctorOfWithUnit = functor₁ ; unit = unit₁ }
  ...
  to-Conditions
    {i} {F}
    record { FunctorOfWithUnit = FunctorOfWithUnit₀ ; unit = unit₀ ; Unit-Homomorphism = Unit-Homomorphism₀ }
  ...
  to-FunctorWithUnit {i} {F} record { FunctorOfWithUnit = FunctorOfWithUnit₀ ; unit = unit₀ } record { Unit-Homomorphism = Unit-Homomorphism₀ } =
    record { FunctorOfWithUnit = FunctorOfWithUnit₀ ; unit = unit₀ ; Unit-Homomorphism = Unit-Homomorphism₀ }
  ...
  to-Body-Eq-From-FunctorWithUnit : {i : Level} → {F : Set i → Set i} → (functor-unit₁ functor-unit₂ : FunctorWithUnit F) → (FunctorOfWithUnit functor-unit₁ ＝ FunctorOfWithUnit functor-unit₂) → ({A : Set i} → unit functor-unit₁ {A} ＝ unit functor-unit₂ {A}) → to-Body functor-unit₁ ＝ to-Body functor-unit₂
  to-Body-Eq-From-FunctorWithUnit
    functor-unit₁ functor-unit₂
    eq-functor eq-unit
    = to-Body-Eq (to-Body functor-unit₁) (to-Body functor-unit₂) eq-functor eq-unit

FunctorWithUnit-Eq : {i : Level} → {F : Set i → Set i} → (functor-unit₁ functor-unit₂ : FunctorWithUnit F) → (FunctorOfWithUnit functor-unit₁ ＝ FunctorOfWithUnit functor-unit₂) → ({A : Set i} → unit functor-unit₁ {A} ＝ unit functor-unit₂ {A}) → functor-unit₁ ＝ functor-unit₂
FunctorWithUnit-Eq
  functor-unit₁ functor-unit₂
  eq-functor eq-unit
